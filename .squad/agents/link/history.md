...
### Issue #19 Session Grain & Durable Replay State (2026-03-26)

- **Session state ownership moved into Orleans without moving transport ownership:** `src\SquadScout.Broker\Sessions\GrainBackedSessionOrchestrator.cs` now fronts the existing relay/pump pipeline with `ISessionGrain`, while `src\SquadScout.Broker\Relay\InMemorySessionRelay.cs` remains the only owner of live PTY transport and stop/input coordination.
- **Durable replay state modeled explicitly:** `src\SquadScout.Broker\Orleans\SessionGrainModels.cs`, `ISessionGrain.cs`, and `SessionGrain.cs` define the persisted session descriptor, replay buffer snapshots, generation/ack metadata, and telemetry envelopes so broker restart no longer erases replay history.
- **Session runtime state can now round-trip through persistence cleanly:** `src\SquadScout.Broker\Sessions\SessionRuntimeState.cs`, `CircularReplayBuffer.cs`, and `SessionTelemetryBuffer.cs` gained snapshot import/export support so the grain can reactivate with the same replay window, last acknowledged broker sequence, and generation-reset boundary data.
- **Operator mode now reflects the Phase 2 cutover:** `src\SquadScout.Broker\Program.cs` swaps `ISessionOrchestrator` to the grain-backed implementation whenever Orleans is enabled, and `/api/orleans/status` reports `session-grains` / `durable-grain` instead of the old bootstrap-only runtime wording.
- **Proof coverage added at the grain seam:** `tests\SquadScout.Broker.Tests\OrleansSessionGrainTests.cs` now covers replay durability across orchestrator reactivation, generation-reset boundary persistence, relay compatibility, and the status snapshot contract for the new runtime mode.
