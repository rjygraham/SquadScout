...
### Issue #16 — Functions/Web PubSub Phase 1 Datapath Gate Pass (2026-03-25)

- **Phase 1 session-group seam closed:** `src\SquadScout.Functions\Upstream\WebPubSubUpstreamHandler.cs` now rejects authenticated upstream envelopes whose `projectId`/`sessionId` cannot form the Phase 1 group contract `session:{projectId}:{sessionId}` before forwarding to the broker, which keeps malformed routing inputs out of the relay path and surfaces contract errors earlier.
