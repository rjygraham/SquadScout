...
- Accepted: Issue #51 — Web PubSub Upstream Authentication. Approve WebPubSubUpstreamAuthenticator implementation with constant-time HMAC-SHA256 signature validation (ce-signature/WebHook-Signature), Managed Identity Easy Auth support scoped to WEBSITE_INSTANCE_ID, comprehensive unit test coverage (10/10 pass). Reviewed by Switch. Ready for merge.
- Accepted: Issue #51 — Authentication implementation decision (Seraph technical spec): Dual-path validator requiring WebHook-Request-Origin match and accepting either trusted Easy Auth principal (TrustedUpstreamPrincipalIds) or valid Web PubSub signature (WebPubSubUpstreamAccessKeys) derived from ce-connectionId. Local/non-Easy-Auth environments must configure at least one access key; Easy Auth deployments must explicitly allow Web PubSub caller via principal list.
- Accepted: PR #50 Merged & Issue #51 Opened. PR #50 (Complete Web PubSub inbound session routing) merged to main, closing issue #14. Security gap identified: WebPubSubUpstreamFunction missing WebHook-Signature validation opened as issue #51 for Seraph. Mandatory follow-up before public deployment; critical for Phase 1 baseline (WS-6). Pattern: review → issue → implementation → testing ensures security controls not skipped.
- Accepted: Issue #14 Unblock Reassessment (Post-PR #48). Do not open PR #14 yet; remains blocked on missing inbound Web PubSub command-ingress path from MAUI to broker. Broker-side routing slice complete; client-side MessagingConnectionService now sends WebPubSubSendToGroupCommand. Coordinate with Seraph/Morpheus on cloud/auth-facing ingress seam before claiming #14 complete. Remaining work: forward inbound Web PubSub events into broker session input route.
- Accepted: PR #49 Merge Watch. Do not merge PR #49 (MAUI session transcript UI, closes #10) until Switch explicit approving review. If approved and diff matches intent, use squash merge. Re-check approval and mergeability immediately before merge; no GitHub checks attached to head commit.
...
- Accepted: Seraph — Aspire ServiceDefaults Architecture. Use shared `src\SquadScout.ServiceDefaults` project multi-targeting `net8.0;net10.0` instead of stock ASP.NET Core-only template. Preserves Functions worker net8.0 compatibility while enabling MAUI OpenTelemetry initialization and broker /health behavior. Aspire AppHost owns broker endpoint orchestration when `ASPNETCORE_URLS` is set; otherwise broker uses `Broker:ListenUrl`. Shared foundation for logging, OTEL, resilience, service discovery. Issue #31 follow-up layers broker-specific tracing and Azure Monitor export.
- Accepted: Seraph — Issue #51 Web PubSub Upstream Authentication Implementation. Dual-path validator in `WebPubSubUpstreamAuthenticator.cs` requires `WebHook-Request-Origin` match and accepts either trusted Easy Auth principal (`TrustedUpstreamPrincipalIds`) or valid Web PubSub signature (HMAC-SHA256, `ce-connectionId`-derived, via `WebPubSubUpstreamAccessKeys`). Treats `ce-signature` as canonical; accepts `WebHook-Signature` alias per PR #50 follow-up. Authenticates before event processing, preventing forged input at broker boundary. Local environments configure access keys; cloud deployments configure principal list. Regression coverage 10/10 upstream handler tests. Build ✅ 90/90 full suite.
- **2026-03-25: Neo — Issue #17 Landing Strategy (Executed).** Reopened Issue #17 to maintain backlog traceability for deferred Phase 1 session telemetry implementation. Created `feature/issue-17-session-telemetry-diagnostics` branch with product/test changes only; committed with "Closes #17"; opened PR #57. Separated `.squad/` bookkeeping from product PR to maintain clean code review surface. All 126 tests pass. Alternatives rejected: creating Issue #58 would fragment backlog; direct-to-main commits bypass review gates.
- **2026-03-25: Link — Issue #17 Diagnostics Architecture.** Treat broker session runtime as authoritative Phase 1 diagnostics source. Store recent envelope headers (redacted), replay/validation/generation-reset events, and sequencing snapshot in `SessionRuntimeState`. Expose via lightweight JSON export: `GET /api/sessions/{sessionId}/telemetry`. Reuse existing `messageId`, `correlationId`, `causationId` fields directly; avoid parallel correlation scheme. Follow-ons: maintain secret-safety on export, keep buffers bounded, future app surfaces consume broker-authored export.
- **2026-03-25: Switch — Issue #17 Acceptance Bar (Approved).** Phase 1 diagnostics seam: broker validation/replay outputs + app's `RecentTraffic` ring buffer. Required exports: input validation exposes generation, sequence fields, and gap status; replay responses expose generation, window bounds, `gapDetected`, correlation IDs. Validation: `BrokerPhase1DatapathGateTests`, `InMemorySessionOrchestratorReplayTests`, `PubSubConnectionServiceTests` all pass; full regression 122/122. Zero external services needed; local analysis sufficient. No blocking gaps remain.
- **2026-03-25: Morpheus — Issue #17 Security & Performance Review (Conditional Approve).** Phase 1 telemetry baseline approved conditioned on gap-detection log hardening. Hardening applied: `InMemorySessionOrchestrator.AcceptClientMessageAsync` now logs `ProjectId`, `Generation`, `MessageId`, `CorrelationId` on gap detection (previously only `SessionId` + sequences). Rationale: multi-project debugging requires generation context. All acceptance criteria verified: sequence/replay context ✅, stable correlation IDs ✅, secret-safe output ✅, minimal Phase 1 scope ✅. Pre-existing non-blocking issues documented. Broker tests 87/87 ✅.
