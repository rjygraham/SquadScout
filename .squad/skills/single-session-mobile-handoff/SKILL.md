...
- **Gate composer on transport truth:** For broker-backed sessions, only enable send affordances when the session is actually `Running` and the live transport is `Connected`; preview/offline mode can remain locally draftable.
...
- Do NOT append local "sent" bubbles for broker-backed sessions when transport is only staged, reconnecting, disconnected, or faulted.
